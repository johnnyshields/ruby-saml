  ...
  module XML
    ...
    module Decryptor
      ...
      def decrypt_document(document, decryption_keys)
        raise ValidationError.new('An EncryptedAssertion found and no SP private keys found to decrypt it') if decryption_keys.empty?

        document_copy = RubySaml::XML.safe_load_nokogiri(document.to_s)
        decrypt_assertion_from_document(document_copy, decryption_keys)
      end

      # Obtains a SAML document with the EncryptedAssertion element decrypted
      # @param document [Nokogiri::XML::Document] A copy of the original SAML document with the encrypted assertion
      # @param decryption_keys [Array] Array of private keys for decryption
      # @return [Nokogiri::XML::Document] The SAML document with the assertion decrypted
      def decrypt_assertion_from_document(document, decryption_keys)
      ...
      def decrypt_nameid(encrypted_id_node, decryption_keys)
        decrypt_node(encrypted_id_node, /(.*<\/(\w+:)?NameID>)/m, decryption_keys)
      end
      ...
      def decrypt_attribute(encrypted_attribute_node, decryption_keys)
        decrypt_node(encrypted_attribute_node, /(.*<\/(\w+:)?Attribute>)/m, decryption_keys)
      end
      ...
      def decrypt_node(encrypt_node, regexp, decryption_keys)
      ...
      def decrypt_node_with_multiple_keys(encrypted_node, private_keys)
