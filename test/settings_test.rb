...
class SettingsTest < Minitest::Test
  ...
  describe "Settings" do
    ...
    describe "#get_certificate_new" do
      it "returns nil when the cert is an empty string" do
        @settings.certificate_new = ""
        assert_nil @settings.get_certificate_new
      end
      ...
      it "returns the certificate when it is valid" do
        ...
        assert @settings.get_certificate_new.kind_of? OpenSSL::X509::Certificate
      end
    ...
    describe 'accessor aliases' do
      {
        idp_cert: :idp_certificate,
        sp_cert: :sp_certificate
      }.each do |aliased_method, original_method|
        it ":#{aliased_method} is aliased to :#{original_method}" do
          @settings.send(:"#{original_method}=", 'Dummy')

          assert_equal 'Dummy', @settings.send(:"#{aliased_method}")
          assert_equal 'Dummy', @settings.send(:"#{original_method}")
        end

        it ":#{aliased_method}= is aliased to :#{original_method}=" do
          @settings.send(:"#{aliased_method}=", 'Dummy')
